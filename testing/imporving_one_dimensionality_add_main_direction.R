...
eig = eigen(sigma_c)
h_pc1 = B[,I1] %*% eig$vectors[,1]
h_pc1
B_ = cbind(h_pc1, B[,I2])
t(B_) %*% B_

mu_pc1 = sum(eig$vectors[,1] * mu_c)
sigma_pc1 = t(eig$vectors[,1,drop=FALSE]) %*% sigma_c %*% eig$vectors[,1,drop=FALSE]
inv_sigma_pc1 = pinv_sympd(sigma_pc1)

d1 = 1
N_h1.x = c_lrnm_cond_posterior_approximation_vec(x, mu_pc1, inv_sigma_pc1, h2, cbind(h_pc1, B[,I2]))
mu_h1.x = N_h1.x[,d1+1]
...
moments_h1.x = c_moments_lrnm_cond_montecarlo_sigma_inverse(x, mu_h1.x, pinv_sympd(sigma_h1.x),
                                                            mu_pc1, inv_sigma_pc1, h2,
                                                            cbind(h_pc1, B[,I2]), Z, mu_centering = rep(0,d1))
moments_h1.x[,d1+1]
moments_h1.x[1:d1,1:d1,drop=FALSE]

x/composition(c(moments_h1.x[,d1+1], h2), cbind(h_pc1, B[,I2]))
composition(c(moments_h1.x[,d1+1], h2), cbind(h_pc1, B[,I2]))[x==0]
...
#
#
# When I1 principal component
#
# > x/composition(c(moments_h1.x[,d1+1], h2), cbind(h_pc1, B[,I2]))
#       x1       x2       x3       x4       x5       x6       x7       x8       x9      x10      x11
# 59.63416  0.00000  0.00000  0.00000  0.00000 59.63416  0.00000  0.00000  0.00000  0.00000 59.63416
# > composition(c(moments_h1.x[,d1+1], h2), cbind(h_pc1, B[,I2]))[x==0]
#          x2          x3          x4          x5          x7          x8          x9         x10
# 0.003990481 0.001856189 0.017368892 0.008691921 0.013085659 0.003167187 0.002839788 0.110554215
