...
M2 = c_m2_lrnm_hermite(x, mu, sigma, Binv = t(Binv), order = 1000) / c_d_lrnm_hermite(x, mu, sigma, Binv = t(Binv), order = 1000)
M2c = M2 - M1^2
...
points(h, f_posterior_approx2, type = 'l', col = 'green', lty = 3)
abline(v = M1)
```
...
```{r}
integrate(function(h) h * f_join(h)/f_join_area_x_fixed, -15, 0)$value +
  integrate(function(h) h * f_join(h)/f_join_area_x_fixed, 0, 15)$value
M1
plot(h, h*f_posterior, type = 'l')
```

```{r}
integrate(function(h) h^2 * f_join(h)/f_join_area_x_fixed, -10, 10)$value
M2
plot(h, h*h*f_posterior, type = 'l')
```
