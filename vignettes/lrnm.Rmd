...
lM = lapply(split(SL3, 1:nrow(SL3)), c_posterior_approximation, mu, solve(sigma), t(MASS::ginv(B)))
...
for(i in 1:nrow(SL3)){
  df_ellip = coda_ellipse(lM[[i]][,3], lM[[i]][1:2,1:2], 0.95)
  df_ellip$state = i
  p = p + 
...
for(i in 1:nrow(SL3)){
  df_ellip = ellipse(lM[[i]][,3], lM[[i]][1:2,1:2], 0.95)
  p = p + 
    geom_path(data = df_ellip, 
